...
# - Subnet tags so the controller can discover public subnets
...
# can assume this role via the OIDC provider.
...
# Tag public subnets so the controller can discover them
# for internet-facing ALBs.
resource "aws_ec2_tag" "subnet_role_elb" {
...
# Service account, pre-created so Helm doesn't manage it
# and we can attach the IAM role ARN annotation for IRSA.
...
resource "helm_release" "alb_controller" {
  ...
  # WAFv2/WAF/Shield are blocked by an org-level SCP in this account.
  # Disable so the controller does not call those APIs on every reconcile.
  set {
    name  = "enableShield"
    value = "false"
  }
